...
static void notify_handler(ACPI_HANDLE node, uint32_t value, void* _ctx) {
    acpi_handle_ctx_t* ctx = _ctx;
    if (ctx->ns_node != node) {
        printf("notify_handler: node %p != %p\n", node, ctx->ns_node);
        return;
    }
    ...
    if (value <= 0x7f) {
        ...
        printf("notify_handler: unknown event value 0x%x\n", value);
        return;
    }
    acpi_evt_t evt = {
        .version = 0,
    ...
    printf("notify_handler: notify type=0x%x value=0x%x\n", type, value);
    ...
    ctx = _ctx;
    while (ctx->node.prev) {
        ctx = containerof(ctx->node.prev, acpi_handle_ctx_t, node);
        if (ctx->notify != MX_HANDLE_INVALID && (ctx->event_mask & type)) {
            mx_channel_write(ctx->notify, 0, &evt, sizeof(evt), NULL, 0);
        }
    }
